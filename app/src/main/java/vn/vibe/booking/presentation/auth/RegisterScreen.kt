...
fun RegisterScreen(
    ...
    LaunchedEffect(state.message) {
        if (!state.message.isNullOrBlank()) { onBackToLogin(); viewModel.clearRegisterTransientState() }
    }
    ...
    Box(modifier = Modifier.fillMaxSize().background(bg).padding(contentPadding).padding(20.dp), contentAlignment = Alignment.Center) {
        Card(colors = CardDefaults.cardColors(containerColor = Color(0xFF0F172A)), shape = RoundedCornerShape(28.dp), modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                ...
                OutlinedTextField(value = name, onValueChange = { name = it }, modifier = Modifier.fillMaxWidth(), label = { Text("Họ và tên") })
                ...
                Button(onClick = { if (name.isBlank() || phone.isBlank() || password.isBlank()) localError = "Vui lòng nhập đầy đủ thông tin" else { localError = null; viewModel.register(name.trim(), phone.trim(), password.trim(), onSuccess = {}) } }, modifier = Modifier.fillMaxWidth(), enabled = !state.isLoading, colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF06B6D4))) { Text("Đăng ký", color = Color.White) }
                if (state.isLoading) CircularProgressIndicator()
                Button(onClick = onBackToLogin, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF111827))) { Text("Đã có tài khoản? Đăng nhập", color = Color.White) }
            }
        }
    }
}
